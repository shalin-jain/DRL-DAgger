...
DROPOUT_MASK = np.array(
    [1, 1, 1, 1, 1, 1, 1, 1]
)  # valid observation indices to apply dropout on
    ...
    for epoch in range(epochs):
        ...
        for _ in range(UPDATE_STEPS):
            ...
            obs_batch = torch.tensor(
                np.array([dataset[i][0] for i in batch_indices]), dtype=torch.float32
            ).to(DEVICE)
            act_batch = torch.tensor(
                np.array([dataset[i][1] for i in batch_indices]), dtype=torch.long
            ).to(DEVICE)
            hidden_batch = torch.stack([dataset[i][2] for i in batch_indices]).permute(
                1, 0, 2
            )
    ...
    for i in range(episodes):
        ...
        while not done and timesteps < EPISODE_LENGTH:
            if isinstance(policy_fn, PPO):
                ...
                with torch.no_grad():
                    action_logits, hidden_state = policy_fn(
...
if __name__ == "__main__":
    for exp in range(5):
        ...
        SAVE_ROOT = os.path.join(
            "dropout_{x}".format(x=DROPOUT_RATE), "seed{x}".format(x=RANDOM_SEED)
        )
        ...
        if TRAIN_NEW_GRU_POLICY:
            ...
            dagger_model = GRUPolicy(
                env.observation_space.shape[0], env.action_space.n
            ).to(DEVICE)
            dagger_model.load_state_dict(
                torch.load(
                    os.path.join(SAVE_ROOT, "dagger_lunarlander_gru.pt"),
                    map_location=DEVICE,
                )
            )
        ...
        if TRAIN_NEW_MLP_POLICY:
            ...
            dagger_model_baseline = MLPPolicy(
                env.observation_space.shape[0], env.action_space.n
            ).to(DEVICE)
            dagger_model_baseline.load_state_dict(
                torch.load(
                    os.path.join(SAVE_ROOT, "dagger_lunarlander_mlp.pt"),
                    map_location=DEVICE,
                )
            )
        ...
        np.save(
            os.path.join(SAVE_ROOT, "dagger_baseline_returns.npy"),
            dagger_baseline_returns,
        )
